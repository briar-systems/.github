...
Do not follow the `^5.4` suggestion in mach 5.4.0's missing-key warning. That range excludes the v5.3.0 seed, so a manifest that follows it builds locally and fails CI. mach tracks the hint as a defect in mach#3572, and the fix will suggest the oldest release of the running major that can read the key. Remove this paragraph once that fix ships.
